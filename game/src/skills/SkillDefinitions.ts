...
export const SKILL_DEFS: Record<SkillId, SkillDef> = {
  multi_shot:       { id:'multi_shot',       name:'マルチショット',         description:'弾数+1（最大5発）',        icon:'icon_star', maxLevel:4, rarity:'common',    applyLevel:(s,lv)=>{ s.bulletCount=Math.min(5,1+lv); } },
  spread_shot:      { id:'spread_shot',      name:'スプレッド弾',           description:'弾が扇状に広がる',        icon:'icon_star', maxLevel:3, rarity:'rare',     applyLevel:(s,_)=>{ s.bulletCount=Math.max(s.bulletCount,3); } },
  ...
  damage_up:        { id:'damage_up',        name:'ダメージUP',             description:'攻撃力+20%',                icon:'icon_star', maxLevel:5, rarity:'common',    applyLevel:(s,lv)=>{ s.damage=Math.floor(s.damage*(1+lv*0.2)); } },
  fire_rate_up:     { id:'fire_rate_up',     name:'攻撃速度UP',            description:'発射間隔-15%',              icon:'icon_star', maxLevel:5, rarity:'common',    applyLevel:(s,lv)=>{ s.fireRateMs=Math.max(80,Math.floor(s.fireRateMs*(1-lv*0.15))); } },
